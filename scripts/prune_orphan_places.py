...
스스로 열리는 화면이 없다 — 지도는 추억을 그리고 사진첩은 원본을 그린다.

그런데 추억을 지울 때 장소는 남겨 두고 있었다. 그래서 추억도 사진도 없는 장소가
...
연결이 없는 노드가 곧 쓰레기인 것은 아니다.
    ...
    인물   가족 공간과 인물 화면이 노드를 그대로 나열한다. 추억에 한 번도
           나오지 않은 사람도 가족이다
    원본   사진첩이 노드를 그대로 나열한다. 추억에 붙지 않은 사진도 사진첩에
           있어야 한다 (붙이는 것은 사람이 고르는 일이다)
    기억   추억 없는 기억 문장은 닿을 수 없지만, 지우는 자리가 이미 있다
