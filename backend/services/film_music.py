...
부르는 곳이 둘이다. Memory Film은 추억 하나로(pick), TV Journey는 추억 여럿을
엮은 여정으로(pick_journey) 무드를 고른다. 낱말 표와 판단 순서는 한 곳에 둔다 —
두 화면이 각자 고르면 같은 추억이 거실에서 다르게 들린다.
...
LLM에 맡기지 않는다. motion_prompt와 같은 이유다 — 같은 추억이 볼 때마다 다른
음악으로 시작하면 그 음악은 이 추억의 것이 아니다. 낱말 표는 사람이 읽고 고칠 수
있고, 왜 그 무드가 나왔는지 화면에 그대로 적을 수 있다.

추억이 가진 말(제목·설명·장소)만 본다. 가족이 남긴 기억 문장은 보지 않는다.
문장 하나가 더 붙었다고 이야기의 음악이 바뀌면 안 되고, "돌아가신 할머니와 갔던
여행"처럼 사람을 가리키는 말이 추억의 성격으로 읽히면 잔치에 추모 음악이 깔린다.
"""
...
# 판단 순서. 한 추억에서 낱말이 여럿 걸릴 때의 우선순위이고(pick), 여정에서 무드가
...
def pick(
    ...
    판단에 순서가 있다. 한 추억이 여러 낱말에 걸리기 때문이다 — "할머니 칠순잔치"와
...
def pick_journey(
    ...
    여정은 추억 하나가 아니라 여럿을 엮은 것이다. 그래서 슬라이드마다 무드를 고르지
    ...
    추억이 하나뿐이면(대기화면에서 OK를 누른 길) pick과 똑같이 답한다. 그쪽 문구가
    "28년 전 기록입니다"처럼 더 구체적이다.

    추억을 하나도 못 찾으면 None을 준다. 화면은 음악 없이 재생한다 — 근거가 없는데
    아무 소리나 얹지 않는다.
    """
    if not events:
        return None
    ...
    return {
        ...
        "reason": f"이 여정의 추억 {len(picked)}개 중 {counts[top]}개가 ‘{MOOD_LABEL[top]}’입니다.",
        "pace": round(pace, 2),
    }
