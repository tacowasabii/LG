...
# 방금 물어본 사건 (답할 사람별로 몇 개만)
#
# 인터뷰를 시작만 하고 답하지 않으면 그래프는 그대로다. 점수도 그대로이므로
# "인터뷰 시작하기"를 다시 눌러도 같은 사건이 나왔다 — 시드된 그래프는 여덟
# 사건 모두 날짜·장소·설명·사진이 채워져 있어 점수가 거의 같고, 같으면 목록의
# 첫 사건이 늘 이겼다. 그것이 1998 부산 가족여행이라 부산 여행만 반복해 물었다.
#
# 그래서 방금 물어본 사건은 다음 선택에서 뒤로 밀어 둔다. 영구히 빼지는 않는다 —
# 한 사건에 여러 사람의 기억이 겹쳐 쌓이는 것이 이 제품이 하려는 일이다.
_recently_asked: dict[str, list[str]] = {}

# 몇 개를 밀어 둘지. 사건 여덟 개 규모에서 셋을 밀어 두면 누를 때마다 다른
# 사건이 나오고, 네 번째 다음에 처음 것으로 돌아온다.
_RECENT_KEEP = 3


def _recent_key(speaker_id: Optional[str]) -> str:
    """답할 사람별로 따로 센다 — 아빠가 물어본 사건이 딸의 차례를 밀어낼 이유가 없다"""
    return speaker_id or "__anon__"


def remember_asked(speaker_id: Optional[str], event_id: Optional[str]) -> None:
    """이 사건을 방금 물어봤다고 적어 둔다

    인터뷰를 **시작하는** 자리에서 부른다. 답변이 들어오기를 기다리지 않는다 —
    시작만 하고 그만둔 경우가 바로 같은 사건이 반복되던 경우다.
    """
    if not event_id:
        return
...
def forget_asked() -> None:
    """밀어 둔 사건을 잊는다 (테스트가 선택 순서를 정할 때 쓴다)"""
...
def _best_event(
    ...
    같으면 목록의 첫 사건이 이겼고, 그래서 auto는 늘 같은 사건을 냈다.
    """
    recent = set(recent)
    ...
    for event in graph_manager.get_events():
        ...
        rank = (
            # 방금 물어본 사건은 뒤로 — 이것이 없어서 같은 사건이 반복됐다
