...
Memory Film을 재생하면 배경 음악이 함께 깔립니다. **음원 파일이 없습니다** —
브라우저가 그 자리에서 만듭니다 (`frontend/src/lib/filmMusic.ts`, Web Audio).
내레이션 낭독이 브라우저 목소리를 쓰는 것과 같은 분업입니다. 키도, 자산도,
비용도 없고 정적 데모에서도 돕니다.
...
- **앱이 만든 소리라고 밝힙니다.** 무드와 고른 근거를 재생 화면에 적고, 끄는
  버튼을 그 옆에 둡니다. 가족이 남긴 기록이 아니라는 사실이 화면에서 읽힙니다
