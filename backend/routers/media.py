...
def _to_list_item(node: dict) -> MediaListItem:
    """미디어 노드를 목록 항목으로. 음성은 화자·추억·질문까지 붙여 내려준다

    음성 재생 화면(홈·인물·채팅·TV)이 "누가 언제 어느 추억에서 무슨 질문에
...
async def delete_media(
    ...
    # 지우기 전에 읽어 둔다. 지운 뒤에는 어느 추억에 붙어 있었는지 알 수 없다.
    ...
    # 추억에 남으면, 이야기가 근거 없이 그 사람을 계속 부른다.
    for event_id in events:
        sync_participants_of_event(event_id)
