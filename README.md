...
기준선이 잘못 적혔으면 환경변수로 덮습니다 — 배포 컨테이너에 들어가 파일을 지울
수 없기 때문입니다.

```bash
MOTION_BASELINE_EVENT_IDS="E01,E02,E03,E04,E05,E06,E07,E08"  # 이것만 제외
MOTION_BASELINE_EVENT_IDS=                                    # 제외 없음(전부 대상)
```

전부를 대상으로 하려면 `MOTION_AUTOGEN_NEW_ONLY=false`로 둡니다.

**왜 안 움직이는지는 `/api/health`가 알려줍니다.**

```json
"motion": { "autogen": true, "new_events_only": true,
            "baseline_events": 8, "attempts_left": 49, "clips": 3 }
```
...
Memory Film과 TV Journey를 재생하면 배경 음악이 함께 깔립니다. **음원 파일이
없습니다** — 브라우저가 그 자리에서 만듭니다 (`frontend/src/lib/filmMusic.ts`,
...
| 무드 | 언제 | 소리 |
|---|---|---|
| `solemn` 낮고 조용하게 | 제사·성묘·장례 | A minor, 아주 느리게, 벨은 드물게 |
| `nostalgic` 회상하듯 | 20년이 넘은 기록 | D minor, 어둡게 |
| `bright` 밝게 | 입학·졸업·여행·놀이 | G major, 빠르게 |
| `warm` 따뜻하게 | 결혼·칠순·명절·가족모임 | F major |
| `calm` 차분하게 | 그 밖의 기록 | C major, 느리게 |

판단에는 **순서**가 있습니다. "추석 가족모임 겸 성묘"처럼 한 기록이 여러 낱말에
...
음악이 없습니다. 낱말 표를 화면에 복사해 두지 않는 쪽을 택했습니다 — 두 곳이
갈라지는 것보다, 추모하는 자리에 근거 없이 아무 소리나 얹는 것이 더 나쁩니다.

두 가지를 지킵니다.

- **가족의 목소리가 이깁니다.** 장면의 실제 음성이나 낭독이 나면 음량을 내립니다
  (끄지 않습니다 — 한 마디마다 꺼졌다 켜지면 그 편집이 목소리보다 더 들립니다)
- **앱이 만든 소리라고 밝힙니다.** 무드와 고른 근거를 재생 화면에 적습니다.
  Film에는 끄는 버튼을 그 옆에 두고, TV에는 두지 않습니다 — 리모컨 6버튼에 새 키를
  만들지 않는다는 규칙이 있어서, 일시정지(▶❙❙)가 음악까지 멈추는 자리입니다.
  가족이 남긴 기록이 아니라는 사실이 두 화면 모두에서 읽힙니다
